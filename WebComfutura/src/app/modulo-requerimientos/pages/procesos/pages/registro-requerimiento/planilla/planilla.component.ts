...
export class PlanillaComponent implements OnInit  , OnDestroy {
  ...
  getCargarCombos(){ 
    this.spinner.show();
    combineLatest([this.combosService.get_tipoDocPersonal(), this.combosService.get_monedas(), this.combosService.get_bancos() , this.combosService.get_cargos()  ])
    .subscribe( ([  _tiposDocumentosPersonal, _monedas, _bancos, _cargos ])=>{     
      this.spinner.hide();   
  ...
  inicializarFormulario(){ 
    this.formParams= new FormGroup({  
      ...
      CargoPersonal: new FormControl('0'), 
 ...
 keyPress(event: any) {
  ...
  changeRucDni(e:any) {
    ...
    if (this.formParams.value.NroDocPersonal == '0') {
      this.formParams.patchValue({"IdBanco": '0', "PubMoneCodigo": '' , "CuentaBanco": '' , "CuentaInterbancarioBanco": '', "CargoPersonal" : '0'}); 
       return ;
    }
    ...
    const personales = this.personalesFiltrada.find( p => p.nroDoc == this.formParams.value.NroDocPersonal );
     ...
     if (personales){
      this.formParams.patchValue({"IdBanco": personales.id_Banco, "PubMoneCodigo": personales.pub_mone_codigo , "CuentaBanco": personales.cuentaCorriente  , "CuentaInterbancarioBanco": personales.CCI, "CargoPersonal" : personales.CargoPersonal  }); 
     }else{
      this.formParams.patchValue({"IdBanco": '0', "PubMoneCodigo": '' , "CuentaBanco": '' , "CuentaInterbancarioBanco": '', "CargoPersonal" : '0'}); 
     }
  ...
  changeBancoMoneda(e:any) {
    ...
    const personales = this.personales.find( p => p.nroDoc == this.formParams.value.NroDocPersonal &&  p.id_Banco == this.formParams.value.IdBanco &&  p.pub_mone_codigo == this.formParams.value.PubMoneCodigo    );
     ...
     if (personales){
      ...
      this.formParams.patchValue({ "CuentaBanco": '' , "CuentaInterbancarioBanco": '', "CargoPersonal" : '0'}); 
     }
 ...
 mostrarViaticos(){ 
  ...
  editarDet({IdTabs, IdRequerimiento, IdTipoTabs, IdTipoMovCaja, DescripcionDetallada, AlmUmedCodigo, CantidadPresupuesto, PrecioPresupuesto, CostoPresupuesto, 
              IdtipoPersonal, NroDocPersonal,CargoPersonal ,IdBanco, PubMoneCodigo, CuentaBanco, CuentaInterbancarioBanco, Estado , Fecha}){ 
      ...
      .subscribe((res:RespuestaServer)=>{     
        ...
        if (res.ok==true) {         
            ...
            this.formParams= new FormGroup({
              ...
              CargoPersonal: new FormControl(CargoPersonal),  
