...
@Component({
  ...
  styleUrls: ['./tab-grupo-solicitud.component.css']
})
export class TabGrupoSolicitudComponent implements OnInit , OnDestroy {
 ...
 mostrarTabGrupos(){ 
  ...
  async saveUpdateDet(){ 
    ...
    if (  this.IdTipoTabsGlobal == 4) { //---CONTRATISTAS SOLO SE PUEDE EDITAR
      if (this.flag_modoEdicion==false ) {
        this.alertasService.Swal_alert('error','Solo se puede editar, por favor elija un item de la tabla para editarlo..');
        return 
      }  
    }
  ...
  editarDet({IdSolicitudTabs, IdSolicitud, IdTipoTabs, IdTipoMovCaja, DescripcionDetallada, AlmUmedCodigo, CantidadSolicitud, PrecioSolicitud, CostoSolicitud, 
    IdTipoPersonal, NroDocPersonal, IdBanco, PubMoneCodigo, CuentaBanco, CuentaInterbancarioBanco, Estado,
              CantidadPresupuesto, PrecioPresupuesto, CostoPresupuesto, ObsRendicion
            }){ 
  ...
  eliminarDet(item: any) {
    ...
    .then((result)=>{
      if(result.value){
        ...
        Swal.showLoading();
        this.registroRequerimientoService.set_eliminar_tabsGrupo(item.IdSolicitudTabs).subscribe((res: RespuestaServer) => {
          Swal.close();
          if (res.ok) {
            this.mostrarTabGrupos();
          } else {
